"""Preprocess genomic alignments into graph-structured feature arrays.

Reads a whitespace-delimited input file of genomic positions and labels,
extracts a context window around each position from a pickled multi-species
alignment dictionary, encodes nucleotides as integers, appends the reverse
complement, and saves the resulting feature matrix (X) and label vector (y)
as NumPy .npy files.

Usage:
    python preprocess_graphs.py <input_path> <chrom> <output_X> <output_y> [alignment_dir]
...
# Build feature arrays: each example is (115 species, context*4+2) uint8
examples = []
targets = []
for i, target in tqdm(zip(indices, y_true)):
    ...
    try:
        ...
        for key in names:
            ...
            # Forward sequence + reverse complement concatenated
            sequence = le.transform(
                sequence_raw[i - context: i + context + 1]
